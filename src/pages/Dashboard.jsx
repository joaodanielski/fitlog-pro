...
export function Dashboard() {
  ...
  async function handleDelete(id) {
    if(!confirm('Tem certeza? Isso apaga todos os exercícios vinculados.')) return
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-50 pb-20">
      ...
      <main className="max-w-md mx-auto p-4 space-y-4">
        ...
        {isCreating && (
          <form onSubmit={handleCreate} className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 animate-in fade-in slide-in-from-top-2">
            <label className="block text-sm text-zinc-400 mb-2">Nome da Ficha</label>
            <div className="flex gap-2">
              ...
              />
              <button type="submit" className="bg-emerald-500 text-zinc-950 font-bold px-4 rounded-lg hover:bg-emerald-400">
                Salvar
              </button>
            </div>
            <button 
        ...
        {/* Lista de Treinos */}
        {loading ? (
          <p className="text-center text-zinc-500 py-10 animate-pulse">Carregando...</p>
        ) : workouts.length === 0 ? (
          ...
          <div className="grid gap-3">
            {workouts.map(workout => (
              <div key={workout.id} className="bg-zinc-900/50 border border-zinc-800/50 p-4 rounded-xl flex items-center justify-between group hover:border-zinc-700 transition-all">
                <div className="cursor-pointer flex-1">
                  <h3 className="font-semibold text-lg text-zinc-100">{workout.name}</h3>
